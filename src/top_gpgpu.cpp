...
Top_gpgpu::Top_gpgpu(uint8_t num_sm)
...
void Top_gpgpu::add_kernel(std::string name, std::string metafile, std::string datafile) {
    std::shared_ptr<kernel_info_t> kernel = std::make_shared<kernel_info_t>(
        m_kernel_cnt, name, metafile, datafile, m_mem->createRootPageTable()
    );
    assert(kernel);
