...
public:
    ...
    PhysicalMemory* pmm;
    Memory(uint64_t max_range) { pmm = new PhysicalMemory(max_range); }
    ~Memory() { delete pmm; }
    // 为任务创建根页表
    uint64_t createRootPageTable() {
        uint64_t root = pmm->findUsable(SV39::PageSize);
        if (root != 0)
            pmm->insertBlock(std::move(Block(root, SV39::PageSize, true).initialize_zero())
            ); // init all PTEs to 0
        else
    ...
    void allocateMemory(uint64_t root, uint64_t v_addr, uint64_t length) {
        assert(
            (v_addr & 0xfff) == 0
        ); // 当前假定v_addr总是一个页的基址，确保物理内存总以页为基本单位来分配
           // 如此，只需检测虚拟地址是否能够正常解析为物理地址即可获悉此页是否已被分配
        if (length == 0)
            return;
        length = (length + SV39::PageSize - 1) / SV39::PageSize
            * SV39::PageSize; // ceil align to page size
    ...
    uint64_t allocateMemory_raw(uint64_t root, uint64_t v_addr, uint64_t length) {
        ...
        for (uint64_t iter = 0ull; iter < length; iter += SV39::PageSize) {
            if (SV39::VAextract((v_addr + iter), 0) != pt_idx[0]) { // may need a new entry in
                                                                    // rootPT
                ...
                if ((pmm->readWord<uint64_t>(root + pt_idx[0] * sizeof(uint64_t)) & SV39::V)
                    ...
                    pmm->writeWord<uint64_t>(
                        root + pt_idx[0] * sizeof(uint64_t), rootpt_entry
                    ); // set rootPT entry
                }
            }
            uint64_t pt1_addr
                = SV39::PTE2PA(pmm->readWord<uint64_t>(root + pt_idx[0] * sizeof(uint64_t)));
            ...
            if (SV39::VAextract(v_addr + iter, 1) != pt_idx[1]) { // may need a new entry in PT1
                ...
                if ((pmm->readWord<uint64_t>(pt1_addr + pt_idx[1] * sizeof(uint64_t)) & SV39::V)
                    ...
                    pmm->insertBlock(
                        std::move(Block(pt2_addr, SV39::PageSize, true).initialize_zero())
                    ); // create and insert a PT2
                    uint64_t pt1_entry = SV39::SetPTE(pt2_addr, SV39::V);
                    pmm->writeWord<uint64_t>(
                        pt1_addr + pt_idx[1] * sizeof(uint64_t), pt1_entry
                    ); // set PT1 entry
                }
            }
            uint64_t pt2_addr
                = SV39::PTE2PA(pmm->readWord<uint64_t>(pt1_addr + pt_idx[1] * sizeof(uint64_t)));
            ...
            if (SV39::VAextract(v_addr + iter, 2) != pt_idx[2]) { // may need a new entry in PT2
                ...
                if ((pmm->readWord<uint64_t>(pt2_addr + pt_idx[2] * sizeof(uint64_t)) & SV39::V)
                    ...
                    uint64_t pt2_entry
                        = SV39::SetPTE(p_addr + iter, SV39::R | SV39::W | SV39::X | SV39::V);
    ...
    uint64_t addrConvert(uint64_t root, uint64_t v_addr) {
        auto pt1_addr = SV39::PTE2PA(
            pmm->readWord<uint64_t>(root + SV39::VAextract(v_addr, 0) * sizeof(uint64_t))
        );
        if (!pt1_addr)
            return 0ull;
        ...
        if (!pt2_addr)
            return 0ull;
        auto p_addr = SV39::PTE2PA(
            pmm->readWord<uint64_t>(pt2_addr + SV39::VAextract(v_addr, 2) * sizeof(uint64_t))
        );
        if (!p_addr)
            return 0ull;
    ...
    template <class T> T readWordPhysical(uint64_t p_addr) { return pmm->readWord<T>(p_addr); }
    template <class T>
