...
int main(int argc, char* argv[]) {
    ...
    auto f_new_kernel = [sim, &kernels, &tasks, &cnt_kernel](
        ...
        if (add_to_task) {
            ...
            SPDLOG_INFO(
                "[main::f_new_kernel] Kernel {} added to task, pagetable=0x{:x}", name,
                kernel->pagetable
            );
            task->add_kernel(kernel);
        } else {
            SPDLOG_INFO(
                "[main::f_new_kernel] Before vmem_create, kernel->pagetable=0x{:x}",
                kernel->pagetable
            );
            auto new_ptroot = ventus_cyclesim_vmem_create(sim);
            SPDLOG_INFO(
                "[main::f_new_kernel] ventus_cyclesim_vmem_create returned pagetable=0x{:x}",
                new_ptroot
            );
            kernel->pagetable = new_ptroot;
            SPDLOG_INFO(
                "[main::f_new_kernel] After assignment, kernel->pagetable=0x{:x}", kernel->pagetable
            );
    ...
    auto f_send_kernel_to_gpu = [sim](
        ...
        SPDLOG_INFO(
            "[main::f_send_kernel_to_gpu] Sending kernel {} to GPU, pagetable=0x{:x}",
        ...
        SPDLOG_INFO(
            "[main::f_send_kernel_to_gpu] After kernel_load_data, kernel->pagetable=0x{:x}",
            kernel->pagetable
        );
