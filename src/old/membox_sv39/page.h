...
public:
    ...
    bool operator<(const Block& another) { return this->end() <= another.addr; }
    bool operator>(const Block& another) { return this->addr >= another.end(); }
    bool operator<(const uint64_t& pos) {
        uint64_t end = this->addr + this->size;
        return end <= pos;
    }
    bool operator>(const uint64_t& pos) { return this->addr > pos; }
    bool operator==(const uint64_t& pos) {
    ...
    int write(uint64_t base, uint64_t length, const void* data) { // 0 <= base < size
        if (base + length > size) {
            log_error(
                "Input 0x%016lx + 0x%016lx is too long for block @0x%016lx", base, length, addr
            );
            return -1;
        }
