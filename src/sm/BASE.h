...
public:
    ...
    static void exec_calc_helper(
        const I_TYPE& ins, int num_thread_active,
...
public:
    ...
    typedef struct {  // hardware block slot in SM, records block(CTA) information
        ...
        int num_warp; // number of warps that are currently running on this SM (reduntant, can be
    ...
    sc_signal<int> last_dispatch_warpid {
        "last_dispatch_warpid"
    }; // 需要设为sc_signal，否则dispatch判断对i的循环边界【i < last_dispatch_warpid +
       // hw_num_warp】会变化
    ...
    std::array<std::pair<int, int>, BANK_NUM>
        REGselectIdx; // 轮询选出哪个了（索引，有这个数据，ready其实没有用了）
    ...
    sc_vector<sc_signal<int32_t>> tolsu_data1 { "emitolsu_data1", hw_num_thread }, // OPC TO LSU
        tolsu_data2 { "emitolsu_data2", hw_num_thread },
        tolsu_data3 { "emitolsu_data3", hw_num_thread };
    bool lsu_ready;
    ...
    sc_signal<bool> emito_simtstk {
        "emito_simtstk"
    }; // 对应join，由于wait_bran的存在，这两个不会同时为1
    sc_signal<bool, SC_MANY_WRITERS> valuto_simtstk {
        "valuto_simtstk"
    }; // 对应beq类，由于wait_bran的存在，这两个不会同时为1
