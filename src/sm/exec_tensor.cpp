    ...
    while (true) {
        ...
        if (emito_tc) {
            if (tc_ready_old == false) {
                std::cout << "tc error: not ready at " << sc_time_stamp() << ","
                          << sc_delta_count_at_current_time() << "\n";
            }
            tc_unready.notify();
            new_data.ins = emit_ins;
            new_data.warp_id = emitins_warpid;
            // std::cout << "SM" << sm_id << " TC_IN receive ins=" << emit_ins.read() << " at " <<
    ...
    while (true) {
        ...
        if (tctmp1.ins.ddd.wxd | tctmp1.ins.ddd.wvd) {
            ...
            case DecodeParams::alu_fn_t::FN_TTF:
                ...
                //     for (int j = 0; j < 2; ++j)
                //         std::cout << std::hex << std::setw(8) << std::setfill('0') <<
                //         tctmp2.rdv1_data[i * 2 + j] << " ";
                //     std::cout << std::endl;
                // }
                break;
            default:
                std::cout << "TC_CALC warning: switch to unrecognized ins" << tctmp1.ins << " at "
                          << sc_time_stamp() << "," << sc_delta_count_at_current_time() << "\n";
                assert(0);
                break;
            }
