...
void Subcore::OPC_FIFO() {
    ...
    opcfifo_t newopcdat;
    while (true) {
        // std::cout << "SM" << sm_id << " OPC_FIFO: finish at " << sc_time_stamp() << "," <<
        // sc_delta_count_at_current_time() << "\n";
        wait();
        // std::cout << "SM" << sm_id << " OPC_FIFO: start at " << sc_time_stamp() << "," <<
        // sc_delta_count_at_current_time() << "\n";
        if (doemit) {
            // std::cout << "opcfifo is popping index " << emit_idx << " at " << sc_time_stamp()
            // <<","<< sc_delta_count_at_current_time() << "\n";
            auto popdat = opcfifo[emit_idx];
            opcfifo.pop(emit_idx); // last cycle emit
            // std::cout << "OPC_FIFO: poped ins " << popdat.ins << "warp" << popdat.warp_id << " at
        ...
        if (dispatch_valid) {
            ...
            if (m_sm_id == 1 && issue_ins.read().currentpc >= 0x80000080
                && issue_ins.read().currentpc <= 0x800000c0) {
                uint32_t global_warp = warpid_convert(m_subcore_id, issueins_warpid);
                std::cout << "[OPC_FIFO] dispatch_valid=1: SM" << m_sm_id << " subcore"
                          << m_subcore_id << " warp" << issueins_warpid
            ...
            if (!opc_in_ready()) // 相当于上一cycle dispatch_ready
            {
                ...
                if (m_sm_id == 1 && issue_ins.read().currentpc >= 0x80000080
                    && issue_ins.read().currentpc <= 0x800000c0) {
                    uint32_t global_warp = warpid_convert(m_subcore_id, issueins_warpid);
                    std::cout << "[OPC_FIFO] ⚠️ SKIP push: SM" << m_sm_id << " subcore"
                              << m_subcore_id << " warp" << issueins_warpid
                              << " (global_warp=" << global_warp << ")"
                              << " pc=0x" << std::hex << issue_ins.read().currentpc << std::dec
                              << " opc_full=" << opc_full << " doemit=" << doemit << " @ "
                ...
                if (_readdata4.ddd.sel_alu3 == DecodeParams::sel_alu3_t::A3_FRS3) {
                    ...
                    in_banktype[2] = 1;
                           ...
                           && _readdata4.ddd.branch != DecodeParams::branch_t::B_R) { // BEQ
                    newopcdat.data[2].fill(_readdata4.imm + _readdata4.currentpc);
                } else if (_readdata4.ddd.sel_alu3 == DecodeParams::sel_alu3_t::A3_PC) { // JALR
                    in_ready[2] = 0;
                    in_valid[2] = 1;
                    ...
                    in_ready[2] = 0;
                    in_valid[2] = 1;
                    in_srcaddr[2] = bank_decode(
                        _readwarpid,
                        (_readdata4.ddd.isvec & (!_readdata4.ddd.readmask)) ? _readdata4.s3
                                                                            : _readdata4.s2
                    );
                ...
                if (m_sm_id == 1 && _readdata4.currentpc >= 0x80000080
                    && _readdata4.currentpc <= 0x800000c0) {
                    ...
                    std::cout << "[OPC_FIFO::push] SM" << m_sm_id << " subcore" << m_subcore_id
                              ...
                              << " op=" << static_cast<int>(_readdata4.op) << " ready=["
                              << in_ready[0] << "," << in_ready[1] << "," << in_ready[2] << "]"
                              << " valid=[" << in_valid[0] << "," << in_valid[1] << ","
                              << in_valid[2] << "]"
        ...
        // std::cout << "OPC_FIFO waiting ev_opc_judge_emit at " << sc_time_stamp() << "," <<
        // sc_delta_count_at_current_time() << "\n";
        wait(ev_opc_judge_emit & ev_regfile_readdata);
        // std::cout << "OPC_FIFO get ev_opc_judge_emit at " << sc_time_stamp() << "," <<
        // sc_delta_count_at_current_time() << "\n";
        ...
        for (int i = 0; i < OPCFIFO_SIZE; i++) {
            for (int j = 0; j < 3; j++)
                if (opc_ready[i][j] == true) {
                    ...
                    if (m_sm_id == 1 && opcfifo[i].warp_id == 1
                        && opcfifo[i].ins.currentpc >= 0x80000088
                        && opcfifo[i].ins.currentpc <= 0x800000c0 && j == 0
                        && opcfifo[i].ins.ddd.sel_alu1 == DecodeParams::A1_RS1) {
                        // 检查是否是标量寄存器（通过 sel_alu1 == A1_RS1）
                        if (opcfifo[i].ins.ddd.sel_alu1 == DecodeParams::A1_RS1) {
                            std::cout << "[OPC_FIFO::store_data] SM" << m_sm_id
                                      << " warp=" << opcfifo[i].warp_id << " pc=0x" << std::hex
                                      << opcfifo[i].ins.currentpc << std::dec
                                      << " op=" << static_cast<int>(opcfifo[i].ins.op)
                                      << " s1=" << opcfifo[i].ins.s1
                                      << " bank_id=" << opcfifo[i].srcaddr[j].bank_id
                                      << " bank_addr=" << opcfifo[i].srcaddr[j].addr
                                      << " banktype=" << opcfifo[i].banktype[j] << " data[0][0]=0x"
                                      << std::hex << opcfifo[i].data[j][0] << std::dec << " @ "
                                      << sc_time_stamp() << std::endl;
                        }
                    }
                    ...
                    if (opcfifo[i].ins.ddd.sel_alu3 == DecodeParams::sel_alu3_t::A3_PC
                        ...
                        // std::cout << "[opc::store_data] JALR ins: pc=0x" << std::hex <<
                        // opcfifo[i].ins.currentpc
                        //           << " s1=" << opcfifo[i].ins.s1 << " bank_id=" <<
                        //           opcfifo[i].srcaddr[j].bank_id
                        //           << " data[2][0]=0x" << opcfifo[i].data[j][0].to_uint() <<
                        //           std::dec
                        //           << " @ " << sc_time_stamp() << "\n";
                    }
                    ...
                    // opcfifo[i].warp_id
                    //      << ", data[" << j << "]=" << printdata_ << ", srcaddr=" <<
                    //      opcfifo[i].srcaddr[j] << " at " << sc_time_stamp() << "," <<
                    //      sc_delta_count_at_current_time() << "\n";
                }
        }
        ev_opc_store.notify();
        // std::cout << "OPC: entry[0]: ins=" << magic_enum::enum_name((OP_TYPE)opcfifo[0].ins.op)
        // << ", tag_valid="<<opcfifo.tag_valid(0)<<", valid={" << opcfifo[0].valid[0] << "," <<
        // opcfifo[0].valid[1] << "," << opcfifo[0].valid[2] << "}, ready={" << opcfifo[0].ready[0]
        // << "," << opcfifo[0].ready[1] << "," << opcfifo[0].ready[2] << "}\n"; std::cout << "
        // entry[1]: ins=" << magic_enum::enum_name((OP_TYPE)opcfifo[1].ins.op) << ",
        // tag_valid="<<opcfifo.tag_valid(1)<<", valid={" << opcfifo[1].valid[0] << "," <<
        // opcfifo[1].valid[1] << "," << opcfifo[1].valid[2] << "}, ready={" << opcfifo[1].ready[0]
        // << "," << opcfifo[1].ready[1] << "," << opcfifo[1].ready[2] << "}\n"; std::cout << "
        // entry[2]: ins=" << magic_enum::enum_name((OP_TYPE)opcfifo[2].ins.op) << ",
        // tag_valid="<<opcfifo.tag_valid(2)<<", valid={" << opcfifo[2].valid[0] << "," <<
        // opcfifo[2].valid[1] << "," << opcfifo[2].valid[2] << "}, ready={" << opcfifo[2].ready[0]
        // << "," << opcfifo[2].ready[1] << "," << opcfifo[2].ready[2] << "}\n"; std::cout << "
        // entry[3]: ins=" << magic_enum::enum_name((OP_TYPE)opcfifo[3].ins.op) << ",
        // tag_valid="<<opcfifo.tag_valid(3)<<", valid={" << opcfifo[3].valid[0] << "," <<
        // opcfifo[3].valid[1] << "," << opcfifo[3].valid[2] << "}, ready={" << opcfifo[3].ready[0]
        // << "," << opcfifo[3].ready[1] << "," << opcfifo[3].ready[2] << "} at " << sc_time_stamp()
        // << "," << sc_delta_count_at_current_time() << "\n";
    }
}
    ...
    while (true) {
        ...
        for (int i = last_emit_entryid; i < last_emit_entryid + OPCFIFO_SIZE; i++) {
            ...
            if (opcfifo.tag_valid(entryidx) && opcitem.all_ready()) {
                ...
                // std::cout << "opcfifo[" << entryidx << "]-" << opcfifo[entryidx].ins << " is all
                // ready, at " << sc_time_stamp() << "," << sc_delta_count_at_current_time() <<
                // "\n";
                ...
                case DecodeParams::SALU:
                    if (salu_ready) {
                        ...
                        findemit = 1;
                        doemit = true;
                        // std::cout << "OPC: salu is ready at " << sc_time_stamp() <<","<<
                        // sc_delta_count_at_current_time() << "\n";
                        emito_salu = true;
                        ...
                        // std::cout << "OPC_EMIT: find all_ready ins " << opcfifo[entryidx].ins <<
                        // "warp" << opcfifo[entryidx].warp_id << " but salu not ready at " <<
                        // sc_time_stamp() << "," << sc_delta_count_at_current_time() << "\n";
                    }
                    break;
                ...
                case DecodeParams::CSR: {
                    if (csr_ready) {
                        ...
                        // std::cout << "opc: emito csr,
                        // tocsr_data1=opcfifo[entryidx].data[0][0]=0x" << std::hex <<
                        // opcfifo[entryidx].data[0][0] << std::dec << "\n";
