        ...
        if (wb_warpid == warp_id) {
            ...
            if (write_s) {
                ...
                std::cout << "SM" << sm_id << " warp " << warp_id << " 0x" << std::hex
                          << wb_ins.read().currentpc << std::dec << " " << wb_ins << " x "
                          << std::setfill('0') << std::setw(3) << rdv1_addr.read() << " "
                          << std::hex << std::setw(8) << rdv1_data[0] << std::dec
                          << std::setfill(' ') << std::setw(0) << " at " << sc_time_stamp() << ","
                          << sc_delta_count_at_current_time() << "\n";
#endif
                if (rdv1_addr != 0)
                    hwarp->s_regfile[rdv1_addr.read()] = rdv1_data[0];
                // if (sm_id == 0 && warp_id == 2 && rdv1_addr == 2)
                //     std::cout << "Warning! " << "SM" << sm_id << " warp " << warp_id << " 0x" <<
