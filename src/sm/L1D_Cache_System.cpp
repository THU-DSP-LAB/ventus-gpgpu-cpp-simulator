...
int L1D_Cache_System::get_l1_hit_count() const { return l1->get_hit_count(); }

int L1D_Cache_System::get_l1_miss_count() const { return l1->get_miss_count(); }
...
int L1D_Cache_System::accept(
    ...
    // 设置请求类型：读或写
    req.m_opcode = (cmd->opcode == L1D_OPCODE_READ) ? LSU_cache_coreReq_opcode::Read
                                                    : LSU_cache_coreReq_opcode::Write;

    // 计算 block 索引（假设所有 lane 地址位于同一 block）
    // 使用系统参数而不是硬编码值，确保与 exec_lsu.cpp 和 l2_tlm.cpp 一致
    uint32_t addr_rebuild
        = (cmd->cache_tag << (2 + log2Ceil(L1D_BLOCK_NUM_WORD) + log2Ceil(L1D_NUM_SET)))
    ...
    // std::cout << "[L1D_Cache_System::accept] cmd->pagetable_root=0x" << std::hex <<
    // cmd->pagetable_root
...
void L1D_Cache_System::forward_rsp() {
    ...
    while (true) {
        ...
        if (sc_core::sc_get_status() != sc_core::SC_RUNNING) {
            ...
            //           << static_cast<int>(sc_core::sc_get_status()) << "），退出线程" <<
            //           std::endl;
        ...
        if (rsp.m_reg_idxw == 99) {
            ...
            for (auto it = callback_table.begin(); it != callback_table.end();) {
                ...
                if (key.second == 0 && orig_cmd->instr.currentpc == rsp.m_pc
                    ...
                    //           << ", pc=0x" << std::hex << rsp.m_pc << std::dec
                    //           << ", callback_table.size()=" << callback_table.size() <<
                    //           std::endl;
        ...
        if (it != callback_table.end()) {
            ...
            // Write hit的响应reg_idxw=0，但可能不在callback_table中（因为write
            // hit不需要等待L2响应） 这种情况下，我们不需要回调，因为write hit已经完成 std::cout <<
            // "[forward_rsp] ℹ️ store write hit响应（rd=0），无需回调: warp="
            ...
            //     std::cerr << std::endl;
            // } else {
            std::cerr << "[forward_rsp] callback_table is empty! This response may be orphaned."
                      << std::endl;
        }
...
L1D_Cache_System::~L1D_Cache_System() {
    // 检查是否有未完成的请求
    if (!callback_table.empty()) {
        std::cerr << "[L1D_Cache_System::~L1D_Cache_System] ⚠️ WARNING: " << callback_table.size()
                  << " pending requests in callback_table:" << std::endl;
        for (const auto& [key, value] : callback_table) {
            const auto& [callback, orig_cmd] = value;
            std::cerr << "  - warp=" << key.first << ", rd=" << key.second << ", pc=0x" << std::hex
                      << orig_cmd->instr.currentpc << std::dec << std::endl;
        }
    }
