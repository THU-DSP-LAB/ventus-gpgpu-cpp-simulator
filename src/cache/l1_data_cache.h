...
public:
    ...
    void write_hit(
        const std::array<uint32_t, hw_num_thread>& fill_data,
        const std::array<bool, LINEWORDS>& write_mask
    ) {
...
public:
        ...
        if (m_tag_array.has_dirty(tag_evict, set_idx_evict, way_idx_evict)) {
            ...
            m_coreReq_pipe1_reg.m_block_addr_evict_for_invORFlu
                = ((tag_evict << (2 + log2Ceil(L1D_BLOCK_NUM_WORD) + log2Ceil(L1D_NUM_SET)))
                   + (set_idx_evict << (2 + log2Ceil(L1D_BLOCK_NUM_WORD))))
                << 2;
                    ...
                    if (status == HIT) {
                        ...
                        if (!m_coreRsp_Q.is_full()) {
                            ...
                            if (pipe1_opcode == Write) {
                                ...
                                m_data_array.write_hit(
                                    set_idx, way_idx, pipe1_r.m_data, pipe1_r.m_block_offset,
                                    pipe1_r.m_mask
                                );
                            } else {
                            ...
                            dcache_2_LSU_coreRsp hit_coreRsp(
                        ...
                        if (pipe1_opcode == Read) {
                            ...
                            if (mshr_status == PRIMARY_AVAIL) {
                                if (!m_memReq_Q.is_full()) {
                                    ...
                                    vec_subentry new_vec_sub = vec_subentry(
                                        pipe1_r.m_reg_idxw, pipe1_r.m_wid, pipe1_r.m_l1id,
                                    ...
                                    memReq_Q_ele new_read_miss = memReq_Q_ele(
                                        Get, 0x0, pipe1_r.m_reg_idxw, pipe1_r.m_l1id,
                                        pipe1_r.m_pagetable_root, pipe1_r.m_instrId, pipe1_r.m_pc,
                            ...
                            if (!m_memReq_Q.is_full()) { //&& !m_coreRsp_Q.is_full()){
                                ...
                                for (int i = 0; i < NLANE; ++i) { // core order to mem order
                                                                  // crossbar
                                    if (pipe1_r.m_mask[i]
                                        == true) { // 在硬件中，这里是offset矩阵转置的独热码
            ...
            if (!m_memReq_Q.is_full()) {
                ...
                if (pipe1_r.LRSCAMO_is_dirty()) {
                    ...
                    memReq_Q_ele new_dirty_back = memReq_Q_ele(
                        PutFullData, 0x0, 0xFFFFF, pipe1_r.m_l1id, pipe1_r.m_pagetable_root,
                        pipe1_r.m_instrId, pipe1_r.m_pc, pipe1_r.m_block_idx,
                    ...
                    memReq_Q_ele new_spe_req = memReq_Q_ele(
                        new_mReq_opcode, new_mReq_param, pipe1_r.m_reg_idxw, pipe1_r.m_l1id,
                        pipe1_r.m_pagetable_root, pipe1_r.m_instrId, pipe1_r.m_pc, pipe1_block_idx,
                        data_memReq, onehot_mask
                    );
        ...
        if (pipe1_r.m_type == 2) { // WaitMSHR
            ...
            if (pipe1_r.invORFlu_has_dirty()) {
                if (!m_memReq_Q.is_full()) {
                    ...
                    memReq_Q_ele new_dirty_back = memReq_Q_ele(
                        PutFullData, 0x0, 0xFFFFF, pipe1_r.m_l1id, pipe1_r.m_pagetable_root,
                        pipe1_r.m_instrId, pipe1_r.m_pc, pipe1_r.m_block_addr_evict_for_invORFlu,
        ...
        if (m_memRsp_Q.m_Q.size() != 0) {
            if (!m_memRsp_pipe1_reg.is_valid()) {
                if (m_memRsp_Q.m_Q.front().d_opcode == AccessAckData) {
                    ...
                    if (memRsp_count++ < 10) {
                        // std::cout << "[L1_cache::memRsp_pipe0_cycle] #" << memRsp_count << "
                        // pc=0x" << std::hex << m_memRsp_Q.m_Q.front().d_pc << std::dec
                        //           << " d_data[0]=0x" << std::hex <<
                        //           m_memRsp_Q.m_Q.front().d_data[0]
                        //           << " d_data[1]=0x" << m_memRsp_Q.m_Q.front().d_data[1] <<
                        //           std::dec
                    ...
                    if (memRsp_count <= 10) {
                        ...
                        //           << " m_fill_data[0]=0x" << std::hex <<
                        //           m_memRsp_pipe1_reg.m_fill_data[0]
                        //           << " m_fill_data[1]=0x" << m_memRsp_pipe1_reg.m_fill_data[1] <<
                        //           std::dec
        ...
        if (m_memRsp_pipe1_reg.is_valid()) {
            ...
            if (type == REGULAR_READ_MISS) {
                ...
                if (!tag_req_current_missRsp_has_sent) {
                    ...
                    if (m_memRsp_pipe1_reg.tag_replace_status == 0) {
                        if (need_replace) {
                            ...
                            // if (m_memRsp_pipe1_reg.m_pc == 0x80000058 ||
                            // m_memRsp_pipe1_reg.m_fill_data[0] == 0x800000a8) {
                            //     std::cout << "[L1_cache::memRsp_pipe1_cycle] FILL pc=0x" <<
                            //     std::hex << m_memRsp_pipe1_reg.m_pc << std::dec
                            //               << " m_fill_data[0]=0x" << std::hex <<
                            //               m_memRsp_pipe1_reg.m_fill_data[0]
                            //               << " m_fill_data[1]=0x" <<
                            //               m_memRsp_pipe1_reg.m_fill_data[1] << std::dec
                            ...
                            // if (m_memRsp_pipe1_reg.m_pc == 0x80000058 || filled_data[0] ==
                            // 0x800000a8) {
                            //     std::cout << "[L1_cache::memRsp_pipe1_cycle] FILL pc=0x" <<
                            //     std::hex << m_memRsp_pipe1_reg.m_pc << std::dec
                            //               << " filled_data[0]=0x" << std::hex << filled_data[0]
                            //               << std::dec
                        ...
                        if (!m_memReq_Q.is_full()) {
                            ...
                            memReq_Q_ele new_dirty_back
                                = memReq_Q_ele( // TODO这里data_array不能在这个周期完成
                                    PutFullData, 0x0, 0xFFFFF, l1id, pagetable_root, instrId, pc,
                                    block_idx_for_req, m_data_array.read_out(), full_mask
                                );
                            m_memReq_Q.m_Q.push_back(new_dirty_back);
                            m_tag_array.allocate(block_idx, way_replace, time);
                            tag_req_current_missRsp_has_sent = true;
                ...
                if (!m_mshr.current_main_0_sub(block_idx)) {
                    assert(
                        !m_mshr.has_secondary_full_return() && "MSHR违规置SECONDARY_FULL_RETURN"
                    );
                    if (!m_coreRsp_pipe2_reg.is_valid()) {
                        bool main_finish = m_mshr.vec_arrange_core_rsp(
                            m_coreRsp_pipe2_reg, block_idx, m_memRsp_pipe1_reg.m_fill_data
                        );
                    ...
                    if (!m_coreRsp_pipe2_reg.is_valid()) {
                        ...
                        dcache_2_LSU_coreRsp secondary_full_return_cRsp(
                            ...
                            cReq_st1_r.m_pagetable_root, cReq_st1_r.m_instrId, cReq_st1_r.m_pc,
                            cReq_st1_r.m_wid, cReq_st1_r.m_mask
                        );
    ...
    void memReq_pipe2_cycle() {
        if (!m_memReq_Q.is_empty() && !m_memReq_pipe3_reg.is_valid()) {
            ...
            if (is_write) {
                ...
                cRsp_blocked_OR_wshr_full
                    = (m_coreRsp_Q.is_full() && mReq.have_to_coreRsp()) || m_wshr.is_full();
            } else if (is_read) {
                wshr_protect = m_wshr.has_conflict(mReq_block_addr);
            }
            ...
            if (!wshr_protect && !cRsp_blocked_OR_wshr_full) { //! mshr_protect &&
                ...
                if (is_write) {
                    ...
                    if (mReq.have_to_coreRsp()) {
                        ...
                        dcache_2_LSU_coreRsp write_miss_coreRsp(
                            99, // regidx
                            dummy_data, mReq.a_l1id, mReq.a_pagetable_root, mReq.a_instrId,
                            mReq.a_pc,
                    ...
                    // For read operations, a_source should already be set to MSHR req_id (which is
                    // reg_idxw) No need to modify it, just ensure it's preserved
                ...
                //           << ", a_pagetable_root=0x" << std::hex << mReq_updated.a_pagetable_root
                //           << std::dec << std::endl;
    ...
    void cycle(cycle_t time) {
        ...
        coreReq_pipe0_cycle(time
        ); // memRsp的优先级比coreReq高，coreReq pipe1必须在memRsp pipe1之前运行
        memRsp_pipe0_cycle(time); // 否则memRsp pipe1的运行会pop memRsp_Q
    }

    void cast_amo_LSU_type_2_TLUH_param(
        enum LSU_cache_coreReq_type_amo coreReq_type, enum TL_UH_A_opcode& TL_opcode,
        uint32_t& TL_param
    ) {
        switch (coreReq_type) {
        case amoadd:
    ...
    void DEBUG_waveform_coreRsp_a_cycle(std::ofstream& waveform_file) {
        ...
        waveform_file << !m_coreRsp_Q.is_empty() << "," << o.m_wid << "," << o.m_reg_idxw << ","
                      << o.m_l1id << "," << o.m_mask[0] << ",";
        waveform_file << (o.m_mask[0] && !o.m_mask[1]) << "," << o.m_data[0] << ",";
    }
    ...
    void DEBUG_waveform_memReq_a_cycle(std::ofstream& waveform_file) {
        ...
        waveform_file << o.is_valid() << "," << o.a_opcode << "," << o.a_param << "," << o.a_source
                      << "," << o.a_l1id << ",";
        waveform_file << o.a_address << "," << o.a_mask[0] << "," << o.a_mask[1] << ","
                      << o.a_data[0];
    }
