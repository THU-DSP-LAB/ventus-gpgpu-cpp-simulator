...
public:
    uint32_t get_block_idx(uint32_t word_addr) { return word_addr >> LOGB2(LINEWORDS); }

    uint32_t get_block_offset(uint32_t word_addr) {
        return ((1 << LOGB2(LINEWORDS)) - 1) & word_addr;
    }

    uint32_t get_tag(uint32_t block_idx) { return block_idx >> LOGB2(NSET); }
    uint32_t get_set_idx(uint32_t block_idx) { return ((1 << LOGB2(NSET)) - 1) & block_idx; }
