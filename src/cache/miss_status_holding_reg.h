...
public:
    ...
    vec_subentry(uint32_t req_id, uint32_t wid, uint32_t l1id,uint32_t pagetable_root, uint8_t instrId,uint32_t pc,
    std::array<bool,NLANE> mask,vec_nlane_t block_offset)//,vec_nlane_t word_offset)
    :m_req_id(req_id), m_wid(wid),m_l1id(l1id),m_pagetable_root(pagetable_root),m_instrId(instrId),m_pc(pc), m_mask(mask), m_block_offset(block_offset){}//,
...
public:
    ...
    vec_entry_target_info(uint32_t req_id, vec_subentry sub): m_req_id(req_id){
        allocate_sub(sub);
    }
...
public:
    ...
    mshr_miss_rsp(enum entry_target_type type, uint32_t req_id, uint32_t l1id,uint8_t instrId,uint32_t pc,
        block_addr_t block_idx):m_type(type), m_req_id(req_id), m_l1id(l1id),m_pagetable_root(m_pagetable_root), m_block_idx(block_idx){}
...
public:
    ...
    void special_arrange_core_rsp(coreRsp_pipe_reg& pipe_reg, uint32_t req_id){
        ...
        dcache_2_LSU_coreRsp new_rsp = dcache_2_LSU_coreRsp(
            req_id,data,the_entry.m_l1id,the_entry.m_pagetable_root,the_entry.m_instrId,the_entry.m_pc,the_entry.m_wid,mask_of_scalar);
    ...
    bool vec_arrange_core_rsp(coreRsp_pipe_reg& pipe_reg, block_addr_t block_idx, std::array<uint32_t, hw_num_thread>& missRsp_line){
        ...
        if(wm_order == 0){
            ...
            dcache_2_LSU_coreRsp new_rsp = dcache_2_LSU_coreRsp(
                ...
                coreRsp_data,
                current_sub.m_l1id,
                current_sub.m_pagetable_root,
                current_sub.m_instrId,
                current_sub.m_pc,
                current_sub.m_wid,
                current_sub.m_mask);
            pipe_reg.update_with(new_rsp);
    ...
    void pop_write_under_readmiss(const block_addr_t block_addr, std::array<uint32_t, hw_num_thread>& write_data, std::array<bool,LINEWORDS>& write_mask){
