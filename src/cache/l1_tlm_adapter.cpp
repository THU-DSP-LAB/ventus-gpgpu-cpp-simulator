    ...
    while (true) {
        ...
        trans->set_command(
            req.a_opcode == TL_UH_A_opcode::Get ? tlm::TLM_READ_COMMAND
                : (req.a_opcode == TL_UH_A_opcode::PutFullData
        ...
        //           << " Added to inflight_map: adapter_req_id=0x" << std::hex << adapter_req_id <<
        //           std::dec
...
tlm::tlm_sync_enum L1_TLM_Adapter::nb_transport_bw(
    ...
    if (phase == tlm::BEGIN_RESP) {
        ...
        if (it == inflight_map.end()) {
            // std::cout << "[L1_TLM_Adapter::nb_transport_bw] ERROR: key=0x" << std::hex << key <<
            // std::dec
            //           << " not found in inflight_map (size=" << inflight_map.size() << ")" <<
            //           std::endl;
        ...
        if (orig_source_it != adapter_source_map.end()) {
            ...
            // std::cout << "[L1_TLM_Adapter::nb_transport_bw] Restored d_source: key=0x" <<
            // std::hex << key
            //           << " -> original d_source=0x" << original_d_source << " -> restored
            //           d_source=0x"
            //           << rspExt->rsp.d_source << std::dec << std::endl;
            // 注意：先不删除映射，等确认响应有效后再删除
        } else {
            // std::cout << "[L1_TLM_Adapter::nb_transport_bw] WARNING: No adapter_source_map entry
            // for key=0x"
        ...
        if (is_store
            && (++adapter_rsp_count <= 100
                || (final_rsp.d_pc >= 0x80000088 && final_rsp.d_pc <= 0x800000c0))) {
            std::cout << "[L1_TLM_Adapter::nb_transport_bw] Store response: pc=0x" << std::hex
                      << final_rsp.d_pc << std::dec << " d_source=" << final_rsp.d_source
                      << " d_instrId=" << final_rsp.d_instrId << " key=0x" << std::hex << key
        ...
        if (final_rsp.d_pc == 0x5555 || final_rsp.d_pc == 0x0) {
            std::cout << "[L1_TLM_Adapter::nb_transport_bw] WARNING: Invalid pc=0x" << std::hex
                      << final_rsp.d_pc << std::dec << ", ignoring this response. key=0x"
