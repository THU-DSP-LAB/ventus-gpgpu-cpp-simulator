...
#include <cstdio>
#include <sstream>
tlm::tlm_sync_enum L2_Cache::nb_transport_fw(int l1id, tlm::tlm_generic_payload& trans, tlm::tlm_phase& phase, 
    ...
    while (true) 
    {
        ...
        if (!req_queue.empty()) 
        {
            ...
            if (reqExt->req.a_opcode == TL_UH_A_opcode::Get && reqExt->req.a_param == 0x0) 
            {
                ...
                if (paddr_block == 0) {
                    char msg[256];
                    std::sprintf(msg, "MMU translate failed: ptroot=0x%x vaddr=0x%x", 
                ...
                cmd->cache_tag
                    = (paddr_block
                       >> (log2Ceil(L1D_BLOCK_NUM_WORD * 4) + log2Ceil(L1D_NUM_SET)));
                ...
                std::cout << "[L2_Cache::process_queue] Sending request to Ramulator: saved_pc=0x" << std::hex << saved_pc << std::dec
                          << ", saved_source=0x" << std::hex << saved_source << std::dec
                          << ", l1id=" << l1id_copy
                          << " @ " << sc_time_stamp() << std::endl;
                int ret = ramulator->request(
                    saved_source, cmd, [=,this](std::unique_ptr<lsu_mem_cmd_t> ret_cmd) mutable {
                        std::cout << "[L2_Cache::ramulator_callback] CALLED: saved_pc=0x" << std::hex << saved_pc << std::dec
                                  << ", saved_source=0x" << std::hex << saved_source << std::dec
                                  << ", l1id=" << l1id_copy
                                  << " @ " << sc_time_stamp() << std::endl;
                        auto* rspExt = new L2MemRspExtension;
                        ...
                        // ret_cmd->data[i] is indexed by thread, but we need to map it to word index based on blockOffset
                        for (int i = 0; i < hw_num_thread; ++i) {
                            if (ret_cmd->mask[i]) {
                                ...
                                std::cout << "[L2_Cache::ramulator_callback] thread=" << i 
                                          ...
                                          << " -> d_data[" << static_cast<int>(blockOffset) << "]=0x" 
                                          << std::hex << rspExt->rsp.d_data[blockOffset] << std::dec << std::endl;
                            }
                        }
                        rspExt->rsp.d_mask.fill(true);
                        trans_copy->set_extension(rspExt);
                        
                        std::cout << "[L2_Cache::ramulator_callback] About to call nb_transport_bw: d_source=0x" << std::hex << rspExt->rsp.d_source << std::dec
                                  << ", d_pc=0x" << std::hex << rspExt->rsp.d_pc << std::dec
                                  << ", l1id=" << l1id_copy
                                  << " @ " << sc_time_stamp() << std::endl;
                        // 调用 nb_transport_bw 将响应发送回 L1
                        tlm::tlm_phase phase = tlm::BEGIN_RESP;
                        sc_time delay = SC_ZERO_TIME;
                        auto status = target_socket[l1id_copy]->nb_transport_bw(*trans_copy, phase, delay);
            ...
            else if (reqExt->req.a_opcode == TL_UH_A_opcode::PutFullData) 
            {
                ...
                rspExt->rsp.d_instrId = reqExt->req.a_instrId;
                 ...
                 for (int i = 0; i < hw_num_thread; ++i) {
                     ...
                     if(i==0) cmd->mask[i] = true;
                     else cmd->mask[i] = false;
                 }
            ...
            } else if (reqExt->req.a_opcode == TL_UH_A_opcode::PutPartialData
                       && reqExt->req.a_param == 0x0) {
                SC_REPORT_INFO(
                    "L2_Cache_Dispatch", "进入分支：putpartialdata"
                );
                ...
                if (++putpartial_rsp_count <= 100 || (reqExt->req.a_pc >= 0x80000088 && reqExt->req.a_pc <= 0x800000c0)) {
                    std::cout << "[L2_PutPartialData_RSP] Sending response: pc=0x" << std::hex << reqExt->req.a_pc << std::dec
                              << " d_source=" << rspExt->rsp.d_source
                              << " d_pc=0x" << std::hex << rspExt->rsp.d_pc << std::dec
                              << " d_instrId=" << rspExt->rsp.d_instrId
                              << " l1id=" << l1id
                              << " @ " << sc_time_stamp() << std::endl;
                }
                ...
                auto mem_stat = m_mem->read(
                    paddr, rspExt->rsp.d_data.data(), cache_building_block::LINESIZE
                );
                if (mem_stat != 0) {
