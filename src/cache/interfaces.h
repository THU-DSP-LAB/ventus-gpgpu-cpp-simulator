...
enum TL_UH_A_PARAM_AMOARITH { MIN = 0x00, MAX = 0x01, MINU = 0x02, MAXU = 0x03, ADD = 0x04 };

enum TL_UH_A_PARAM_AMOLOGIC { XOR = 0x00, OR = 0x01, AND = 0x02, SWAP = 0x03 };
...
public:
    ...
    dcache_2_L2_memReq(
        enum TL_UH_A_opcode opcode, uint32_t param, uint32_t source_id, uint8_t instrId,
        uint32_t pc, uint32_t block_idx, std::array<uint32_t, hw_num_thread> data,
...
public:
    memReq_Q_ele(
        enum TL_UH_A_opcode opcode, uint32_t param, uint32_t source_id, uint32_t l1id,
        uint32_t pagetable_root, uint8_t instrId, uint32_t pc, uint32_t block_idx,
    ...
    bool have_to_coreRsp() { return need_coreRsp; }
...
public:
    ...
    bool is_empty() { return m_Q.size() == 0; }
...
struct L2_2_dcache_memRsp : cache_building_block {
    L2_2_dcache_memRsp() { }
    L2_2_dcache_memRsp(
        enum TL_UH_D_opcode opcode, uint32_t req_id, std::array<bool, LINEWORDS> mask,
        std::array<uint32_t, hw_num_thread> data, uint8_t instrId, uint32_t pc
    )
        : d_opcode(opcode)
        , d_source(req_id)
        , d_mask(mask)
...
public:
    ...
    dcache_2_LSU_coreRsp(
        uint32_t reg_idxw, vec_nlane_t data, uint32_t l1id, uint32_t pagetable_root,
        uint8_t instrId, uint32_t pc, uint32_t wid, std::array<bool, NLANE> mask
    )
        : m_wid(wid)
        , m_reg_idxw(reg_idxw)
        , m_l1id(l1id)
...
public:
    ...
    LSU_2_dcache_coreReq(
        enum LSU_cache_coreReq_opcode opcode, uint32_t type, uint32_t wid, uint32_t req_id,
        uint32_t block_idx, uint32_t l1id, uint32_t pagetable_root, uint8_t instrId, uint32_t pc,
        vec_nlane_t perLane_addr, std::array<bool, NLANE> mask, vec_nlane_t data,
...
public:
    ...
    void set_dirty_for_invORFlu() { m_tag_has_dirty = true; }

    void unset_dirty_for_invORFlu() { m_tag_has_dirty = false; }

    bool invORFlu_has_dirty() { return m_tag_has_dirty; }

    void set_dirty_for_LRSCAMO() { m_chosen_tag_is_dirty = true; }

    void unset_dirty_for_LRSCAMO() { m_chosen_tag_is_dirty = false; }
...
inline std::ostream& operator<<(std::ostream& os, const LSU_2_dcache_coreReq& req) {
    ...
    for (bool m : req.m_mask)
        os << m << " ";
    os << "], block_offset=[";
    for (auto b : req.m_block_offset)
        os << "0x" << std::hex << b << " ";
    os << "], word_offset=[";
    for (auto w : req.m_word_offset)
        os << "0x" << std::hex << w << " ";
    os << "], data=[";
    for (auto d : req.m_data)
        os << "0x" << std::hex << d << " ";
    os << "]}";
    return os;
}
