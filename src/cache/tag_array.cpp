...
void tag_array::probe_in(uint32_t block_idx){
    ...
    //需要替换时，返回true
    bool tag_array::need_replace(uint32_t& tag_replacement, uint32_t& way_replacement, uint32_t block_idx){
        uint32_t set_idx = get_set_idx(block_idx);
    ...
    */
    ...
    /*bool tag_array::issue_memReq_write(memReq_Q& mReq_Q, meta_entry_t& line_to_issue, uint32_t set_idx){
        if (!mReq_Q.is_full()){
            uint32_t block_addr = (line_to_issue.tag() << LOGB2(NSET) + set_idx) << 2;
