...
int sc_main(int argc, char *argv[])
{
    ...
    {
        if (BASE_impl[0]->m_hw_warps[i] != nullptr)
        {
            ...
            for (int j = 0; j < 32; j++)
            {
                sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->CSR_reg[j], "CSR.data(" + std::to_string(j) + ")");
            }
            sc_trace(tf[i], clk, "Clk");
            sc_trace(tf[i], rst_n, "Rst_n");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->jump, "jump");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->jump_addr, "jump_addr");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->branch_sig, "branch_sig");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->fetch_valid, "fetch_valid");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->fetch_valid2, "fetch_valid2");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->pc, "pc");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->fetch_ins, "fetch_ins");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->decode_ins, "decode_ins");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->dispatch_warp_valid, "dispatch_warp_valid");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->ibuf_empty, "ibuf_empty");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->ibuf_swallow, "ibuf_swallow");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->ibuftop_ins, "ibuftop_ins");
            ...
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->simtstk_jump, "simtstk_jump");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->simtstk_jumpaddr, "simtstk_jumpaddr");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->current_mask, "current_mask");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->vbran_sig, "vbran_sig");
            ...
            for (int j = 0; j < 32; j++)
            {
                sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->s_regfile[j], "s_regfile.data(" + std::to_string(j) + ")");
            }

            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->v_regfile[0][0], "v_regfile(0)(0)");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->v_regfile[1][0], "v_regfile(1)(0)");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->v_regfile[2][0], "v_regfile(2)(0)");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->v_regfile[3][0], "v_regfile(3)(0)");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->v_regfile[4][0], "v_regfile(4)(0)");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->v_regfile[5][0], "v_regfile(5)(0)");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->v_regfile[6][0], "v_regfile(6)(0)");
            sc_trace(tf[i], BASE_impl[0]->m_hw_warps[i]->v_regfile[7][0], "v_regfile(7)(0)");
