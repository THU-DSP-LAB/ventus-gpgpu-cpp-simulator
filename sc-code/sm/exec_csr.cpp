    ...
    while (true)
    {
        ...
        if (csrtmp1.ins.ddd.wxd | csrtmp1.ins.ddd.wvd)
        {
            ...
            case CSRRW_:
                ...
                cout << "SM" << sm_id << " warp " << csrtmp1.warp_id << " write CSR[0x" << std::hex << csr_addr << "]=0x" << WARPS[csrtmp1.warp_id]->CSR_reg[csr_addr]
            ...
            case CSRRS_:
                ...
                // cout << "CSRRS, t=" << std::hex << t << ", csrSdata1=" << csrtmp1.csrSdata1 << std::dec << ", ins.s1=" << csrtmp1.ins.s1 << "\n";
                cout << "SM" << sm_id << " warp " << csrtmp1.warp_id << " write CSR[0x" << std::hex << csr_addr << "]=0x" << WARPS[csrtmp1.warp_id]->CSR_reg[csr_addr]
                     << " by ins pc=0x" << csrtmp1.ins.currentpc << csrtmp1.ins << " at " << sc_time_stamp() << "," << sc_delta_count_at_current_time() << "\n";
                break;
            case CSRRC_:
            ...
            case SETRPC_:
                ...
                csrtmp2.data = 0;
                cout << "SM" << sm_id << " warp " << csrtmp1.warp_id
                     << std::hex << " 0x" << csrtmp1.ins.currentpc << " " << csrtmp1.ins
                     << std::hex << " CSR[0X80c]=" << WARPS[csrtmp1.warp_id]->CSR_reg[0x80c] << std::dec << " at " << sc_time_stamp() << "," << sc_delta_count_at_current_time() << "\n";
                break;
            default:
